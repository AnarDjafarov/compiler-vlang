...
int main(void) /* --------------------------------------------------------------------------------- */
{
	...
	assignScalarVector(v1, 6, 2*x);
	int tempArr0[] = {1,1,2,2,3,3};

	assignVectorVector(v2, tempArr0, 6);
	printf("%d\n", pointCalc(v2,v1,6,-1) );
	y = v2[4];
	i = 0;
	...
	int* tempDaynamicArr2 = vectorVectorOp(v1, '+', v2, 6);

	assignVectorVector(v3, tempDaynamicArr2, 6);
	int tempArr3[] = {2,1,0,2,2,0};
	printf("%d\n", v2[(pointCalc(v3,tempArr3,6,-1)/10)] );
	int a[3];
	int tempArr4[] = {10, 0, 20};

	assignVectorVector(a, tempArr4, 3);
	i = 0;
	...
	assignScalarVector(z, 4, 10);
	int tempArr7[] = {2, 4, 6, 8};

	int* tempDaynamicArr8 = vectorVectorOp(z, '+', tempArr7, 4);

	int* tempDaynamicArr9 = vectorScalarOp(tempDaynamicArr8,'/', 2, 4);

	assignVectorVector(z, tempDaynamicArr9, 4);

	int* tempDaynamicArra = vectorScalarOp(z,'-', 3, 4);
	int tempArr13[] = {2, 3, 4, 5};

	int* tempDaynamicArrc = vectorVectorOp(tempDaynamicArra, '+', tempArr13, 4);

	assignVectorVector(z, tempDaynamicArrc, 4);
	printArray(z, 4);
